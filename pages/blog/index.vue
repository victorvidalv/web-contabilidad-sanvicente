  ...
  export default {
    ...
    async mounted() {
      try {
        ...
        this.posts = res
          .slice(0, 9) // Limita el número de publicaciones a 9
          .sort((a, b) => b.id - a.id); // Ordena los posts de mayor a menor ID
      } catch (error) {
    ...
    filters: {
      formatDate(value) {
        if (value) {
          return new Date(value).toLocaleDateString('es-CL'); // Formatea la fecha al estilo chileno
        }
      }
    }
  }
